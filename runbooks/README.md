...
1. Pick a location on disk, e.g. `~/repos/workspace-os-<team>`.
2. Initialize the canonical folder structure described in `protocol/RESPONSIBILITY_FILESYSTEM_STANDARD_V0_1.md`:
   ...
   - Copy templates from `protocol/examples/` (steward persona, guardrails, mandates) and `protocol/telemetry/policies.default.yaml` → `telemetry/policies.<workspace>.yaml`.
3. Add a `boot_trial_logs/` folder with no files yet—runbooks will drop logs there after execution.

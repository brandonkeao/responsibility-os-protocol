...
- Act as **Agent Coach** (meta-agent) when present: audit Responsibilities for standards compliance (folder layout, context hygiene, visualization/output contracts), keep an agent registry/scorecard, and propagate best practices across the workspace.
- Support **cross-responsibility orchestration**: maintain a hub responsibility (e.g., “data_manager”) that aggregates outputs, correlates signals, and issues RFAs to/from specialized Responsibilities instead of ad-hoc peer-to-peer calls.

...
- **UTB content rules**: single task goal; top 5–8 governing principles; only necessary structures/constraints; style normalized to persona + GIP. If UTB synthesis fails, block execution and log memory/telemetry.
- **mem0**: optional external memory; never the System-of-Record. If used, follow pack `mem0` hints (max_memories, max_tokens_hint, tags), inject summarized memory below UTB and above user input, and log outages/overrides.

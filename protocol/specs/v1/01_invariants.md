...
5. **Auditability** – Every action produces a traceable tuple `(kernel_decision_id, guardrail_clause_id, memory_pointer)` and is mirrored to the centralized system event log; this anchors reviewers to both Kernel and Guardrails artifacts.
...
14. **Ops Observability Only** – Layer-3 (ops/infra) components may log, route, or mirror state but must not mutate cognition, authority, or memory ordering. When in doubt, work routes back through RFA/Task primitives under Guardrails supervision.
15. **Agentic Primacy** – Responsibilities own their own Tasks; external Responsibilities interact via RFAs and may only spawn Tasks within their own containers after acceptance. Coordination happens through RFAs and the centralized event log, not implicit cross-writes.
